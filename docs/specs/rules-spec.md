...
- **`main_customer_lost`**: contraparte de facturas **emitidas** con factura en ≥ `recurrence_months` (6) de los últimos `window_months` (12) y ≥ `min_share` (20 %) de la facturación emitida de la ventana, que no factura en los últimos `absence_months` (3) meses, t incluido. Evento en el primer mes que cumple; no se repite mientras siga ausente (mismo episodio). La ventana cuenta **toda la historia de facturas**, también anterior al primer mes de la rejilla: un cliente que se fue justo antes del inicio de las features da evento igual (revisión del 19 sep, noche).
- **`large_maturity`**: contrato de `debt_schedule_config` cuyo `last_payment_date` cae en los `maturity_days` (90) días siguientes al fin del mes t, con `outstanding_balance` ≥ `maturity_min_outflow_months` (1,0) × media de 3 meses de `outflows_eur`. Evento en el primer mes dentro de la ventana, una vez por contrato; solo existe con cuadro de amortización (87 contratos).
- **`expensive_new_debt`**: producto de `debt_products` dado de alta con tipo de contrato por encima de la referencia «cara». La referencia es el percentil `expensive_percentile` (75) **congelado en el `RulesModel`** (`expensive_rate_p75`, que `xray-score` calcula con `events.rate_reference` a `train_until`) y el que usan ingest y `xray-prescore-packs` vía `EventsConfig.expensive_rate_threshold`: un pack no se compara consigo mismo. Sin umbral congelado, el percentil se calcula sobre la tabla recibida pero solo con los contratos existentes a fin del mes del alta (sin mirar el futuro; los productos del cuadro sin alta en `debt_products` cuentan como preexistentes) y exigiendo ≥ `expensive_min_contracts` (4). Evento en el mes del alta; sin contrato no hay evento (nulo, no estimado: `interest_charge` no sirve, plan §5).

Los cuatro puntos de entrada que disponen de tablas crudas construyen la tabla y la pasan a `rules.run(events_ext=…)`: `export_web.build_scores`, `prescore.score_pack` (con las tablas unificadas del propio pack y el umbral del modelo), `api/main.py` `ingest` (idem) y `xray-score`/`xray-evals` con `--events-from-data` (`--events` externo sigue teniendo prioridad). El watch resultante dura 3 meses por `rules.watch` (§6); el campo lleva el código y la web lo traduce con `watchMeta` (`bands.ts`) en chips, alertas y la explicación del score.

Medido (19 sep, noche): 241 meses-empresa con watch activo en la historia —172 cliente perdido, 61 vencimiento, 8 deuda cara; cada evento enciende tres meses— y 22 de las 1.265 empresas llevan watch en su último mes. Evaluación (`evals.watch_metrics`, clave `watch`): P(mes rojo en (t, t+3]) con watch frente a sin watch, sobre las filas evaluables del test (no rojas en t, con los tres meses siguientes): **17,1 % frente a 7,8 %** (41 filas con watch, 0,8 % de las evaluables; `kinds` cuenta meses con watch, ~3 por evento). Baja pero positiva: el objetivo ~60 % del plan §2 no se cumple y se reporta tal cual.

### 12.3 Métricas del método en el pack (`metrics.json`)

`xray-export-web` escribe, junto a `scores.json`, `web/lib/xray/dataset/metrics.json` (`--metrics`, `--metrics-out`, `--metrics-name`): el subconjunto fijo `MethodMetrics` (pydantic, `allow_inf_nan=False`) de `artifacts/evals/metrics.json` — `score_model`, `generated_from`, `train_until`, `test_months`, `n_rows`/`n_companies`/`n_events`, `auc6_own`, `auc6_external`, `auc1_external`, `lead_time` (n_events, cuotas crossing/late/chronic/no_history, mediana y cuartiles del cruce, cutoff), `persistence` (base_rate, horizon, p_red_given_red k=1…6), `directionality` (solo `p_red_t6_given_*`), `projection` y `watch`. Si `metrics.json` no existe o no valida avisa y borra un `metrics.json` anterior del pack — no se sirven cifras de una corrida vieja; la web lo lee en runtime y tolera que el fichero falte (`getMethodMetrics` devuelve `null`). En la web: `MethodMetricsSchema` (Zod) lo valida campo a campo, `GET /api/xray/metrics` lo sirve, el panel «Cómo anticipa el score» lo muestra con su ventana de prueba y la tool `get_method_metrics` de Eve lo devuelve tal cual.

## 13. Retadores sobre PD6 (20 sep 2026, rama ML-experiments)
