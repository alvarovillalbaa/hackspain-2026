...
**Resolución de la revisión final (19 sep, noche; commits `7169c2c` y siguientes):** los menores anteriores quedaron cerrados (`_bin_index` compartido, carga estricta de modelo inconsistente con test, mensajes uniformes, `demopacks` captura `(ValueError, TypeError)` y avisa con motivo, `coverage_80_by_*` siempre presentes con tramo `unknown`, guardia de columnas `proj_*`, pack regenerado). La revisión completa (dos ejes, spec y calidad) añadió y se resolvió: coerción numérica en los eventos (el ingest no devuelve 500 por celdas malas), `large_maturity` una vez por episodio de contrato, `expensive_new_debt` con referencia congelada en el modelo (`expensive_rate_p75`, `events.rate_reference` a `train_until`) en vez del pool del propio pack, pool «a fecha» que solo cuenta contratos existentes a fin del mes del alta, `main_customer_lost` cuenta la historia de facturas anterior a la rejilla, `watch_metrics` con denominador único (filas evaluables; `kinds` = meses con watch), `write_method_metrics` borra el `metrics.json` viejo si la fuente falta, `generated_from` relativo y posix, consolas cp1252 (`sys.stdout.reconfigure` utf-8 en los CLI), `metrics.json` leído en runtime en la web, watch traducido en alertas y explicación, abanico de grupo = media ponderada de los abanicos de las filiales. **Quedan abiertos (menores, documentados en el PR):** tests de evals con fixtures degeneradas; el pack pre-scored y el catálogo difieren ±0,1 pt por el perfil de rangos (convención del slice 11).
