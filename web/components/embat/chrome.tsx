...
import { Sidebar } from "@/components/embat/sidebar";
...
export function EmbatShell({ children }: { children: ReactNode }) {
  useWatchSlackSync();
  return (
    <div className={`${embatUiClass} flex min-h-dvh bg-white text-black`}>
      <Sidebar />
      <main className="min-w-0 flex-1 overflow-auto p-[50px] max-lg:p-6">
        {children}
      </main>
    </div>
  );
