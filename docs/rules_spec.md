...
- **`expensive_new_debt`**: producto de `debt_products` dado de alta con tipo de contrato por encima del percentil `expensive_percentile` (75) de los tipos de `debt_schedule_config`. Evento en el mes del alta; sin contrato no hay evento (nulo, no estimado: `interest_charge` no sirve, plan §5). El percentil se calcula sobre la tabla que recibe el llamador: en cartera es la referencia de los 87 contratos; en ingest/packs es la del propio pack, así que con menos de `expensive_min_contracts` (4) contratos con tipo el evento no se emite.
