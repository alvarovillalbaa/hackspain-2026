...
export default defineTool({
  description:
    "Aggregate metrics for one company (cash, debt, overdue, fees). Use only to cite figures that support an alert. " +
    "Never add or compare figures across currencies.",
  inputSchema: z.object({
    company_id: z.string().regex(/^COMP_\d{4}$/, "e.g. COMP_0058"),
  }),
  label: { start: ({ company_id }) => `Métricas ${company_id}` },
